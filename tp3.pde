...
void setup() {
  ...
  for ( int i=0; i<cantImagenes; i++ ) {
    ...
    fondos[i] = loadImage("fondos/fondo"+nf(i, 8),"jpg");
  ...
  textos[37] = "FIN";
  textos[38] = "pero entonces él penso -Yo no soy tonto, hay algo raro aqui- fue entonces que se decidio a exponer a los estafadores por como eran en verdad al emperador pero sin que se dieran cuenta";
  textos[39] = "Él fingio que podia ver el traje y alago el trabajo de los estafadores, ellos se lo creyeron sin pensarlo mucho ,luego de ello el ministro aporvecho ";
  textos[40] = "y le informo al emperador la verdad que habia descubierto sobre los estafadores y los encerraron en la carcel por el resto de sus vidas";
  textos[41] = "FIN";
  textos[42] = "El emperador uso un traje que le gusto más y que pensó que seria mejor para la ocasión y dejo el traje nuevo para una ocasión especial.";
  textos[43] = "Y así marchó mientras todo el pueblo lo admiraba y decía cumplidos sobre su traje";
  textos[44] = "FIN";
  textos[45] = "El emperador al darse cuenta que fue un tonto y que no merecía su cargo decidió irse del pueblo y dejando todo atrás sin nuevo traje y sin ropa ";
  textos[46] = "Y así el emperador siguió marchando sin parar lejos de todo solo, sin sus súbditos y sin sus sirvientes, diciendo que era un tonto";
  textos[47] = "FIN";
}
...
void draw() {
  if ( estado == 0 ) {
    ...
    dibujaBotonRectangular( width/4*3, height/3, 120, 80, "PENSARLO BIEN");
     } else  if ( estado == 12 ) {
    dibujaPantalla( fondos[3], textos[38] );
    dibujaBotonRectangular( width/4*3, height/3, 120, 80, "SIGUIENTE");
    dibujaBotonRectangular( width/4, height/3, 120, 80, "ANTERIOR");
  }else  if ( estado == 13 ) {
    dibujaPantalla( fondos[3], textos[39] );
    dibujaBotonRectangular( width/4*3, height/3, 120, 80, "SIGUIENTE");
    dibujaBotonRectangular( width/4, height/3, 120, 80, "ANTERIOR");
  }else  if ( estado == 14 ){
    dibujaPantalla( fondos[3], textos[40] );
    dibujaBotonRectangular( width/4*3, height/3, 120, 80, "SIGUIENTE");
    dibujaBotonRectangular( width/4, height/3, 120, 80, "ANTERIOR");
  }else  if ( estado == 15 ){
    dibujaPantalla( fondos[3], textos[41] );
    dibujaBotonRectangular( width/4*3, height/3, 120, 80, "SIGUIENTE");
    dibujaBotonRectangular( width/4, height/3, 120, 80, "ANTERIOR");
  }else  if ( estado == 0 ) {
    dibujaBotonRectangular( width/4, height/3, 120, 80, "IGNORAR");
   } else  if ( estado == 16 ) {
    ...
    dibujaBotonRectangular( width/4*3, height/3, 120, 80, "USAR OTRO TRAJE");
  } else  if ( estado == 30 ) {
    dibujaPantalla( fondos[5], textos[42] );
    dibujaBotonRectangular( width/4*3, height/3, 120, 80, "SIGUIENTE");
    dibujaBotonRectangular( width/4, height/3, 120, 80, "ANTERIOR");
  } else  if ( estado == 31 ) {
    dibujaPantalla( fondos[5], textos[43] );
    dibujaBotonRectangular( width/4*3, height/3, 120, 80, "SIGUIENTE");
    dibujaBotonRectangular( width/4, height/3, 120, 80, "ANTERIOR");
  } else  if ( estado == 32 ) {
    dibujaPantalla( fondos[5], textos[44] );
    dibujaBotonRectangular( width/4*3, height/3, 120, 80, "SIGUIENTE");
    dibujaBotonRectangular( width/4, height/3, 120, 80, "ANTERIOR");
  } else  if ( estado == 0 ) {
    dibujaBotonRectangular( width/4, height/3, 120, 80, "USAR EL NUEVO TRAJE");
  }else  if ( estado == 33 ) {
    ...
    dibujaBotonRectangular( width/4*3, height/3, 120, 80, "DARSE CUENTA");
    } else  if ( estado == 42 ) {
    dibujaPantalla( fondos[6], textos[45] );
    dibujaBotonRectangular( width/4*3, height/3, 120, 80, "SIGUIENTE");
    dibujaBotonRectangular( width/4, height/3, 120, 80, "ANTERIOR");
  }else  if ( estado == 43 ) {
    dibujaPantalla( fondos[6], textos[46] );
    dibujaBotonRectangular( width/4*3, height/3, 120, 80, "SIGUIENTE");
    dibujaBotonRectangular( width/4, height/3, 120, 80, "ANTERIOR");
  }else  if ( estado == 44 ) {
    dibujaPantalla( fondos[6], textos[47] );
    dibujaBotonRectangular( width/4*3, height/3, 120, 80, "SIGUIENTE");
    dibujaBotonRectangular( width/4, height/3, 120, 80, "ANTERIOR");
  }else  if ( estado == 0 ) {
    dibujaBotonRectangular( width/4, height/3, 120, 80, "SEGUIR");
  } else  if ( estado == 45 ) {
